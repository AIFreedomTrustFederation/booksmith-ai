...
const pillars = [
  {
    ...
    body: "Writers keep creative control, approval authority, manuscripts, exports, and publishing decisions.",
  },
  {
    title: "Local-first AI",
    body: "Core workflows are designed for self-hosted and open-source runtimes without required paid APIs.",
  },
  {
    title: "Inspectable publishing",
    body: "Markdown, DOCX, EPUB, LaTeX, metadata, build logs, and launch assets remain visible and editable.",
  },
];

const workflow = [
  "Create or import the manuscript",
  "Lock voice, lore, audience, and creative boundaries",
  "Revise chapters with continuity-aware AI support",
  "Typeset proofs with local LaTeX and open fonts",
  "Export the publishing packet for AI Freedom Trust Publishing",
  "Launch with blurbs, metadata, campaigns, and provenance logs",
];
...
const federationLinks = [
  {
    ...
    label: "AI Freedom Trust Federation",
  },
  {
    href: "https://github.com/AIFreedomTrustFederation/booksmith-ai",
    label: "Booksmith AI Repository",
  },
];
...
export default function Home() {
  return (
    <main className="min-h-screen overflow-hidden bg-[#101811] text-[#f8fbf4]">
        ...
        <div className="mx-auto grid max-w-7xl gap-12 px-6 pb-20 pt-14 lg:grid-cols-[minmax(0,1fr)_480px] lg:px-8 lg:pb-28 lg:pt-20">
          <div>
            <p className="inline-flex rounded-full border border-[#bde8bd]/30 bg-white/10 px-4 py-2 text-sm font-semibold text-[#d7f5d4] shadow-2xl backdrop-blur">
              Author-first writing, editing, publishing, and launch infrastructure
            </p>
            ...
            <p className="mt-7 max-w-3xl text-lg leading-8 text-[#dbe9d7] sm:text-xl">
              Booksmith AI is the creative foundry for the AI Freedom Trust Federation:
              a local-first studio where authors draft, revise, typeset, package,
              and launch books with inspectable AI support and human authority at
              the center.
            </p>
          ...
          <div className="rounded-[2rem] border border-white/15 bg-white/10 p-4 shadow-2xl backdrop-blur">
            <div className="rounded-[1.5rem] border border-[#d8e1d1] bg-[#f7fbf4] p-5 text-[#17201a]">
              ...
              <div className="mt-6 space-y-3">
                {studioModules.slice(0, 5).map((module, index) => (
                  <div
                    className="flex items-center justify-between rounded-2xl border border-[#dfe8da] bg-white px-4 py-3 shadow-sm"
                    key={module}
                  >
                    <span className="font-semibold">{module}</span>
                    <span className="rounded-full bg-[#eef7ea] px-2.5 py-1 text-xs font-bold text-[#3f674b]">
                      {index < 3 ? "Ready" : "Planned"}
                    </span>
                  </div>
                ))}
              </div>
        ...
        <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[420px_minmax(0,1fr)]">
          <div>
            ...
            <h2 className="mt-4 text-4xl font-black tracking-tight">
              From first draft to launch packet.
            </h2>
            <p className="mt-5 leading-8 text-[#607064]">
              Booksmith connects the manuscript, story bible, editorial logic,
              typesetting source, launch campaign, and publishing imprint metadata
              into one traceable production system.
            </p>
          </div>
      ...
      <section className="bg-[#17201a] px-6 py-20 text-white lg:px-8" id="ecosystem">
        <div className="mx-auto max-w-7xl rounded-[2rem] border border-white/10 bg-white/5 p-8 shadow-2xl lg:p-12">
          <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_360px]">
            <div>
              ...
              <h2 className="mt-4 text-4xl font-black tracking-tight sm:text-5xl">
                Booksmith AI is the authoring studio. AI Freedom Trust Publishing
                is the imprint path.
              </h2>
              <p className="mt-6 max-w-3xl leading-8 text-[#cfe7d4]">
                The landing page now points visitors into the product studio while
                making the ecosystem role clear: author sovereignty, local-first AI,
                open publishing formats, and traceable release operations.
              </p>
            </div>
            <div className="space-y-3">
              {federationLinks.map((link) => (
                <a
                  className="block rounded-2xl border border-white/10 bg-white/10 px-5 py-4 font-bold text-white transition hover:bg-white/15"
                  href={link.href}
                  key={link.href}
                >
                  {link.label}
